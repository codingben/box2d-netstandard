...
```c#
Body World.CreateBody(in BodyDef def)
Joint World.CreateJoint(in JointDef def)
```
...
```c#
Fixture BodyCreateFixture(in FixtureDef def)
...
```c#
Fixture Body.CreateFixture(in Shape shape, float density = 0f)
```
