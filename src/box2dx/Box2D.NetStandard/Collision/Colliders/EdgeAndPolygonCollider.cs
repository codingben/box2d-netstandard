...
namespace Box2D.NetStandard.Collision {
  internal class EdgeAndPolygonCollider : Collider<EdgeShape, PolygonShape> {
    internal override void Collide(out Manifold manifold, in EdgeShape edgeA, in Transform xfA, in PolygonShape polygonB, in Transform xfB) {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static EPAxis ComputePolygonSeparation(in Vector2 normal, in TempPolygon polygonB, in Vector2 v1, in Vector2 v2, in float radius, in Vector2 upperLimit, in Vector2 lowerLimit) {
      EPAxis axis;
