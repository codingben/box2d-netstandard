    ...
    public void Initialize(Manifold manifold, XForm xfA, float radiusA, XForm xfB, float radiusB) {
      ...
      switch (manifold.Type) {
        ...
        case ManifoldType.FaceA: {
          Vector2 normal     = Common.Math.Mul(xfA.R, manifold.LocalPlaneNormal);
          Vector2 planePoint = Common.Math.Mul(xfA,   manifold.LocalPoint);

          // Ensure normal points from A to B.
          Normal = normal;

          for (int i = 0; i < manifold.PointCount; ++i) {
            Vector2 clipPoint = Common.Math.Mul(xfB, manifold.Points[i].LocalPoint);
        ...
        case ManifoldType.FaceB: {
          Vector2 normal     = Common.Math.Mul(xfB.R, manifold.LocalPlaneNormal);
          Vector2 planePoint = Common.Math.Mul(xfB,   manifold.LocalPoint);

          // Ensure normal points from A to B.
          Normal = -normal;

          for (int i = 0; i < manifold.PointCount; ++i) {
            Vector2 clipPoint = Common.Math.Mul(xfA, manifold.Points[i].LocalPoint);
