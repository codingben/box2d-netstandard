...
namespace Box2DX.Collision
{
	public partial class Collision
	{
		...
		public static float FindMaxSeparation(ref int edgeIndex, PolygonShape poly1, XForm xf1, PolygonShape poly2, XForm xf2)
		{
			...
			Vector2 d = Common.Math.Mul(xf2, poly2._centroid) - Common.Math.Mul(xf1, poly1._centroid);
			...
			PolygonShape polyA, XForm xfA, PolygonShape polyB, XForm xfB)
		{
			...
			Vector2 localNormal = Vector2.Normalize(Vectex.Cross(dv, 1.0f));
			Vector2 planePoint = 0.5f * (v11 + v12);

			Vector2 sideNormal = Vector2.Normalize(Math.Mul(xf1.R, v12 - v11));
			Vector2 frontNormal = Vectex.Cross(sideNormal, 1.0f);
